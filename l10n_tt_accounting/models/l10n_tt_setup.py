...
class L10nTTSetup(models.Model):
    ...
    setup_notes = fields.Text(default='This applies Trinidad & Tobago defaults for Odoo Community + Odoo Mates Full Accounting.')
    ...
    def action_apply_basic_setup(self):
        ...
        return {
            ...
            'params': {
                ...
                'message': _('Country, fiscal country, TTD currency, VAT 12.5%, chart of accounts, journals, selected bank accounts, fiscal positions, and default taxes were configured.'),
                'sticky': False,
                'type': 'success',
            }
        }
    ...
    def _create_tax(self, name, amount, tax_type, account, description=None):
        ...
        vals = {
            ...
            'tax_group_id': self._tax_group('Trinidad & Tobago VAT').id,
            'description': description or name,
            'price_include_override': 'tax_excluded' if 'price_include_override' in Tax._fields else False,
        }
        vals = {k: v for k, v in vals.items() if k in Tax._fields or k in ('name', 'amount', 'amount_type', 'type_tax_use', 'company_id', 'tax_group_id', 'description')}
        if not tax:
            tax = Tax.create(vals)
        else:
            tax.write(vals)
