...
class L10nTTSetup(models.Model):
    ...
    def action_apply_basic_setup(self):
        for setup in self:
            ...
            setup._ensure_compliance_profile()
    ...
    def action_run_readiness_audit(self):
        for setup in self:
            ...
            compliance = self.env['l10n.tt.compliance'].search([('company_id', '=', company.id)], limit=1)
            add('Compliance profile exists', bool(compliance), 'Missing')
            if compliance:
                add('BIR Number configured', bool(compliance.bir_number), 'Missing')
                add('VAT Registration Number configured', bool(compliance.vat_registration_number or not compliance.vat_registered),
                    'Missing for VAT registered company' if compliance.vat_registered else 'Not VAT registered')
                add('PAYE Registration Number configured', bool(compliance.paye_registration_number), 'Missing')
                add('NIS Employer Number configured', bool(compliance.nis_employer_number), 'Missing')
                add('Company Registration Number configured', bool(compliance.company_registration_number), 'Missing')
    ...
    def _ensure_compliance_profile(self):
        Compliance = self.env['l10n.tt.compliance'].sudo()
        profile = Compliance.search([('company_id', '=', self.company_id.id)], limit=1)
        if not profile:
            profile = Compliance.create({
